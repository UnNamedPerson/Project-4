...
void saveDatabase(HASHTABLE temp, char *new_name){
// create a FILE typed pointer
    FILE *file_pointer;

// open/create the txt file for saving the database
    file_pointer = fopen(new_name,"a+");
...
int main(int argc, const char * argv[]) {
    HASHTABLE temp;
    char *name_of_database = "save_database_1.txt";
    ...
    //saves the database
    saveDatabase(temp,name_of_database);
    
/*** Make sure! You have already called "insert" & "saveDatabase" functions before loading database ***/
    //loads the database
    loadDatabase(name_of_database, temp);
    displayRelations(temp);
